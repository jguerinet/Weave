...
typealias PlatformColumn = Pair<String, Int>
...
open class Weave {
    ...
    /** Comma separated list of platform keys (one platform is one column and therefore one key) */
    open val platformsKeys by lazy { config.platformsColumnName }
     ...
     *  the key and platforms (an empty list if not found)
     */
    open fun parseHeaders(
        headers: Array<String?>,
        onColumn: (index: Int, header: String) -> Unit
    ): Pair<Int, List<PlatformColumn>> {
        // Keep track of which columns hold the keys and the platform
        var keyColumn = -1
        val platformColumns = mutableListOf<PlatformColumn>()
        val platforms = platformsKeys.split(",")
        ...
        headers.forEachIndexed { index, s ->
            ...
            when {
                ...
                platforms.any { it.equals(s, ignoreCase = true) } -> platformColumns.add(PlatformColumn(s, index))
            }
        ...
        return Pair(keyColumn, platformColumns)
    }

    /**
     * Parses the csv from the [reader] using the [headers]. Determines the key and platforms using the [keyColumn] and
     *  [platformColumns], and delegates the parsing to the caller with [onLine]. Parses the headers itself using the
    ...
    open fun parseCsv(
        ...
        headers: Array<String?>,
        keyColumn: Int,
        platformColumns: List<PlatformColumn>,
        ...
        while (currentLine != null) {
            ...
            val platformColumn = platformColumns.firstOrNull { it.first.equals(platform.name, ignoreCase = true) }
            if (platformColumn != null) {
                // If there's a platform column, parse it and check if this String is for this platform
                val platformValue = currentLine[platformColumn.second] as? String
                if (
                    !platformValue.equals("true", ignoreCase = true) &&
                    !platformValue.equals("x", ignoreCase = true)
                ) {
                    // If the platform column is not equal to true or X, skip it
                    lineNumber++
                    currentLine = reader.read(*processors)
                    continue
                }
    ...
    open fun downloadStrands(config: StringsConfig, source: Source): List<BaseStrand>? {
        ...
        val (keyColumn, platformColumns) = parseHeaders(headers) { index, header ->
        ...
        return parseCsv(source, reader, headers, keyColumn, platformColumns) { lineNumber, key, line ->
